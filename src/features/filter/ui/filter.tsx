...
type FolterProps = {
  ...
  setActiveFilters: React.Dispatch<
    React.SetStateAction<Partial<Record<FilterId, number[]>>>
  >;
};
...
export const Filter = memo(
  ({ activeFilters, setActiveFilters }: FolterProps): JSX.Element => {
    const [currentFilter, setCurrentFilter] = useState(
      Object.keys(FILTERS)[0] as FilterId
    );
          ...
          {FILTERS[currentFilter]?.elements.map((element) => (
            <button
              ...
              className={cn(
                ...
                {
                  [classes.active]: activeFilters[currentFilter]?.includes(
                    element.elementId
                  ),
                },
                {
                  [classes.active]:
                    currentFilter === "other" &&
                    element.filterId &&
                    activeFilters[element.filterId]?.some(
                      (filter) => filter === element.elementId
                    ),
                }
              )}
              onClick={() =>
                handleActiveFiltersClick(
                  element.filterId || currentFilter,
                  element.elementId
                )
              }
            >
