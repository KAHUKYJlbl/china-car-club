...
export const Adds = memo(({ currentSpecification, setCurrentSpecification, techs }: AddsProps) => {
  ...
  if (!adds || !currency || !name || !shorts || addsLoadingStatus.isLoading) {
    return (
  ...
  return (
    <>
      <div className={classes.wrapper}>
        <h3 className={classes.header}>Дополнительные товары</h3>
      ...
      {adds.groups.map((group, index) => (
        ...
        >
          <div className={classes.headerWrapper}>
            <p className={classes.groupHeader}>{group.name}</p>
            ...
            <div className={classes.buttonWrapper}>
              {group.items.map((item) => (
                <button
                  ...
                  key={item.id}
                  className={cn(classes.secondary, { [classes.current]: activeItems.includes(item.id) })}
            ...
            <div className={classes.descriptionWrapper}>
              {
                <p>
                  <b>
                    {getCurrentItem(activeItems, group.items)!.description.slice(
                      0,
                      getCurrentItem(activeItems, group.items)!.description.indexOf(":") + 1
                    )}
                  </b>
          ...
          <div className={classes.priceWrapper}>
            ...
            <button
              aria-label={addItems.includes(getCurrentItem(activeItems, group.items)!.id) ? "Удалить" : "Добавить"}
              className={cn(classes.priceButton, {
                [classes.added]: addItems.includes(getCurrentItem(activeItems, group.items)!.id),
              })}
              onClick={() => addItemHandler(group.items)}
            >
              {addItems.includes(getCurrentItem(activeItems, group.items)!.id) ? "Удалить" : "Добавить"}
            </button>
          </div>
        </div>
      ))}
