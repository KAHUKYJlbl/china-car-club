...
export const Calculation = memo(
  ({ calculation, currency }: CalculationProps) => {
      ...
      >
        <div className={classes.info}>
          ...
          <img
            ...
            }/specification/${calculation.specification.id}.jpg`}
          />
          ...
          <p className={classes.model}>
            <span className={classes.bold}>
              ...
              <br />
              {calculation.specification.series.name.ru ||
                calculation.specification.series.name.ch}
            </span>

            <span>
              {calculation.specification.name.ru ||
                calculation.specification.name.ch}
            </span>

            <span className={classes.grey}>
              {calculation.specification.year} поколение • 2024 выпуск
            </span>
          </p>
          ...
          <p className={classes.properties}>
            {[
              FILTERS.engine!.elements.find(
                (element) =>
                  element.elementId ===
                  calculation.specification.parameters.engineTypeId
              )?.name || null,
              FILTERS.body!.elements.find(
                (element) =>
                  element.elementId ===
                  calculation.specification.parameters?.bodyTypeId
              )?.name || null,
              `${
