...
>("Specifications/fetchSpecifications", async ({ modelId, filters, manufacturerId }, { extra: axios }) => {
  try {
    const {
      data: { specificationsBySeriesId },
    } = await axios.get<ManufacturersWithSpecificationsType>(
      [
        APIRoute.Catalog,
