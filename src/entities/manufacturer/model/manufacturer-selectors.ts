...
export const getFiltersLoadingStatus = createSelector(
  (state: State): FetchStatus => state[NameSpace.Manufacturer].filtersLoadingStatus,
  (status) => ({
    isIdle: status === FetchStatus.Idle,
    isLoading: status === FetchStatus.Pending,
    isSuccess: status === FetchStatus.Success,
...
export const getModelsList = createSelector(
  [getManufacturers, (_state: State, id: number | null) => id],
  (manufacturers, id) =>
    id
      ? manufacturers?.manufacturers
          ...
          ?.seriesList.map((model) => ({
            ...
            id: model.id,
            sublistLength: manufacturers.specificationsCountBySeries
...
export const getManufacturerByModel = createSelector(
  [getFullList, (_state: State, id: number | null) => id],
  (manufacturers, id) => {
    if (id) {
