...
export const Offer = ({ offer }: OfferProps) => {
  ...
  return (
    <div className={classes.wrapper}>
      ...
      <p className={classes.comment}>{offer.comment}</p>

      <div className={classes.contacts}>
        <Link
          to={getContactUrl(getPriorityContact(offer.dealer.contacts, true).typeId).concat(
        ...
        {isAddContacts ? (
          getPriorityContact(offer.dealer.contacts, false).map((contact) => (
            <Link
              key={contact.contact}
              to={getContactUrl(contact.typeId).concat(
                contact.contact.startsWith("@") ? contact.contact.slice(1) : contact.contact,
              )}
              target={getUrlTarget(contact.typeId)}
              className={cn(classes.contactButton, classes[getContactProvider(contact.typeId)])}
            >
              <svg
                width="16"
                height="16"
